...
const myComplain = async (req,res)=>{
  try {
    // console.log("in my complain");
    ...
    const user = await Student.findById(decodeData.studentId);
    // console.log(user);
    
    
    const complain = await Complaint.find({studentId:user._id});
    return res.status(200).json({data:complain});
  } catch (error) {
    return new Error(error);
  }
}
